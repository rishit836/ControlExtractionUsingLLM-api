...
def controls_request(uuid_of_file:str):
    ...
    if status:
        ...
        path = os.path.join("extracted_controls",uuid_of_file+"_validated.json")
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)   
                data = clean_and_validate_json(data)
                return data
        else:
            ...
            if os.path.exists(path):
                with open(path, "r", encoding="utf-8") as f:
                    data = json.load(f)   
                    data = clean_and_validate_json(data)
                    return {"msg":"extraction is done the controls are now being validated."}"""
