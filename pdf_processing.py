...
Task: Extract individual definitive controls or regulatory requirements from the provided text into a raw JSON list: [{"control_id", "control_title", "control_desc"}].
...
# validating the controls in the json file extracted using llm.
...
def extract_controls(pdf_path,uuid_of_file):
    ...
    for i, page_text in enumerate(text_page):
        ...
        status_msg[str(uuid_of_file)] = f"--- Processing Page {i+1} ---"
    ...
    with open(os.path.join(output_folder, f"{uuid_of_file}.json"), "w", encoding="utf-8") as f:
        json.dump(final_clean_data, f, indent=2, ensure_ascii=False)
    status_msg[str(uuid_of_file)] = f"extraction done in {minutes}m {seconds}s.found {len(final_clean_data)} ({total_duration//len(final_clean_data)}s/per control)"
